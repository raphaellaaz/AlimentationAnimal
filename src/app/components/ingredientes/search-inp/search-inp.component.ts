...
export class SearchInpComponent implements OnInit{
    ...
    ngOnInit(){
      this.ingredienteService.getAllIngredientes().subscribe(data => {
        this.optionsIngredientes = data.map(( ingredientes: IngredienteModel) => ingredientes);
    ...
    private _filter(value: string): IngredienteModel[] {
      ...
      return this.optionsIngredientes.filter(option => option.Nombre_Ingrediente.toLowerCase().includes(filterValue));
    }
    ...
    removeItem(item: IngredienteModel) {
      this.selectedIngredientes = this.selectedIngredientes.filter(option => option.Nombre_Ingrediente !== item.Nombre_Ingrediente);
    }
